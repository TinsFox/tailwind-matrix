...
export function SearchProvider({ children }: { children: React.ReactNode }) {
  ...
  const onInput = useCallback(
    (e: { key: React.SetStateAction<null> }) => {
      setIsOpen(true);
...
function Hit({ hit, children }: React.PropsWithChildren<IHit>) {
  return (
    <Link href={hit.url}>
      <a
        className={clsx({
...
export function SearchButton({
  children,
  ...props
}: React.PropsWithChildren<ISearchButton>) {
...
function isEditingContent(event: { target: any }) {
